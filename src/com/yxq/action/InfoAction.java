...
public class InfoAction extends InfoSuperAction {	
	public String ListShow(){
		...
		if(currentP==1){     		//设置显示第1页信息的SQL语句
			...
			sqlFreeSub="SELECT  * FROM tb_info i WHERE (info_type = ?) AND (info_state = '1') AND (info_payfor = '0') AND (info_date < (SELECT MIN(info_date) FROM (SELECT TOP "+top2+" (info_date) FROM tb_info WHERE (info_type = i.info_type) AND (info_state = '1') AND (info_payfor = '0') ORDER BY info_date DESC) AS mindate)) ORDER BY info_date DESC limit 0,"+top1;
//			sqlFreeSub="SELECT TOP "+top1+" * FROM tb_info i WHERE (info_type = ?) AND (info_state = '1') AND (info_payfor = '0') AND (info_date NOT IN (SELECT TOP "+top2+" info_date FROM tb_info WHERE (info_type = i.info_type) AND (info_state = '1') AND (info_payfor = '0') ORDER BY info_date DESC)) ORDER BY info_date DESC";         //另一种实现分页查询的SQL语句
	...
	public String SearchShow() throws UnsupportedEncodingException{
		...
		if(currentP==1){     		//显示第1页信息的SQL语句
			sqlSearchSub="SELECT * FROM tb_info WHERE ("+subsql+opname+"?) ORDER BY info_date DESC limit 0,"+top1;
		}
		else{						//显示除第1页外，其他指定页码信息的SQl语句
			int top2=(currentP-1)*top1;
			sqlSearchSub="SELECT  * FROM tb_info WHERE ("+subsql+opname+"?) AND (info_date < (SELECT MIN(info_date) FROM (SELECT info_date FROM tb_info WHERE "+subsql+opname+"'"+param+"' ORDER BY info_date DESC limit 0,"+top2+") AS mindate)) ORDER BY info_date DESC limit 0,"+top1;
